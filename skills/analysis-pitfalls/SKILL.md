...
Never guess field names. Use only the exact field paths returned by `mcp__credible__get_context`. A plausible-sounding name that does not exist in the model will produce an error — or worse, silently reference the wrong field.
