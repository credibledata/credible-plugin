---
name: credible-analysis
description: Workflow for answering data questions with Credible's get_context and execute_query MCP tools covering structured retrieval, Malloy query construction, verification, and answer delivery. Use for…
---
# Credible Analysis
...
2. **Entity drill-down** — pick the top 1-3 sources from step 1 and fire parallel `get_context` calls, each scoped to one source (`environment` + `package` + `model_path` + `source`) with `dimension` / `measure` / `view` / `dimensional_value` targets tailored to what that source likely contains. Even when you know the entity name, use a descriptive `search_text` — don't just echo the name.
   ...
   **REQUIRED — never use a dimension, measure, or view you haven't drilled into.** A name seen anywhere — a step-1 source summary, another entity's docstring, the user's question, prior conversation, your own memory — is a *pointer*, not documentation. Before using an entity in a query, you MUST have seen it returned in an entity drill-down and read its own docstring, which is the only place that reveals what it computes, its grain, units, null handling, and caveats — none of which can be inferred from the name or a sibling's docs.

   **Validating or inspecting a known entity by name.** When you already know the exact field path (e.g., the user named it, another entity's docstring referenced it, or you remember it from prior work), issue a `get_context` call with the source scope plus `entity_name` set to the field path and `search_text: null` on the matching target type. This confirms the entity exists and returns its docstring *and* Malloy code without burning a semantic search — useful both for validation and for understanding how a dimension, measure, or view is implemented (especially views, where you often need the exact filters applied).

   **Read the source's own docstring, not just the entity's.** Beyond the per-entity docstring (above), the **source's** docstring often defines its grain, the universe of rows it represents, how joins behave, and source-level filters or assumptions that affect every query rooted on it. Factor both the entity and source docstrings into query construction and verification.
